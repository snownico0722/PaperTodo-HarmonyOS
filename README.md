...
- 对齐 Windows v2.2 的四套纸片配色：`warm / ink / forest / rose`（暖纸 / 墨 / 林 / 霞）
- 四套配色分别提供 Light / Dark palette，并即时通过 `AppStorage` 广播到已打开纸片和胶囊
- 配色只属于表现层；颜色统一由 `PaperPalette.ets` 计算，不增加 density / DPI 资源目录，也不改变窗口几何
...
- 状态栏实时列出纸片，点击条目切换显隐
- 管理中心只在用户主动选择“管理”时打开
- 管理中心提供：主题、四套配色、Markdown、待办视觉大小、完成项置底、完成后清除、待办关联笔记、关联标题显示、现有胶囊设置等
...
- 旧状态缺少 `colorScheme` 时直接归一为 `warm`，不需要额外 schema 迁移链
...
- 继续改善 Markdown 浏览态视觉与行内表现；优先使用 ArkUI `Text + Span` 原生能力补粗体、斜体、删除线、行内代码和链接，不为了完整 CommonMark 引入重依赖
...
当前 v2.2 对齐分支在 `b57134d` 已通过 HAP 编译和完整 APP 打包。
