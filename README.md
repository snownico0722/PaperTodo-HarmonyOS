...
当前应用版本为 `3.3.8`（`versionCode: 3030800`），数据 schema 为 `7`，目标 SDK 为 HarmonyOS `6.1.0(23)` / API 23。工程当前只声明 `2in1` 设备类型；桌面窗口、Hover、Topmost 和系统状态栏入口都需要在 HarmonyOS PC / 2in1 上验收。

> v3.3 冻结范围的主要功能已经接入，3.3.8 是桌面可靠性修复候选，不代表胶囊已通过真机验收。修复覆盖胶囊就绪交接、原生掩码时序、队列分页、失败恢复和保存失败阻止退出。正式包仍只能由受信分支上的 AGC release 签名流水线交付；PR 构建和逻辑单测不能替代真机验收。参见 [3.3.8 验收清单](docs/V338_RELIABILITY_ACCEPTANCE.md)。

## 产品语义

日常操作仍以独立纸片为中心，不新增纸片管理主界面。当前启动器 / 任务栏入口由 `ManagerAbility` 打开独立设置窗口，后台恢复由 `EntryAbility` 承担：

1. `EntryAbility` 只负责无可见内容的启动和恢复。
2. 已显示的 Todo / Note 以各自独立纸片窗口恢复；已折叠且启用侧边胶囊的纸片由整列胶囊子窗口恢复；子窗口失败时恢复为展开纸片。
3. 支持 `SystemCapability.PCService.StatusBarManager` 的 2in1 设备会安装 PaperTodo 系统状态栏入口，并启动隐藏的驻留 Ability。
4. 设置是独立窗口，包含常规、视觉、快捷键三个分区，不承担新建和纸片管理职责；始终提供“恢复全部纸片”，故障时提供胶囊重试 / 保存重试。
...
- 拖动柄、置顶、点击编辑标题
- 跨窗口拖动关联柄仍暂时隐藏；保留已有纸片关联和非拖拽关联能力
- Note 的 `MD` 外部打开入口
- `＋✓` 新建 Todo、`＋✎` 新建 Note
...
| 能力 | 当前状态 | 说明 |
| --- | --- | --- |
| 启动 / 任务栏入口与独立纸片恢复 | 代码已接入，真机复核中 | ManagerAbility 打开设置并保留恢复入口；EntryAbility 为后台宿主 |
...
| 单屏右侧侧边胶囊 | 代码已接入，真机复核中 | 由 `MasterCapsuleAbility` 的应用子窗口统一承载；按物理屏幕右边界定位，垂直分页限制在工作区内 |
| 主胶囊 | 代码已接入，真机复核中 | 与普通侧边成员共用一个原生子窗口；`▾ 收起` / `▸ N 个` 只控制成员行的隐藏和显示，不改变纸片折叠状态 |
...
- 侧边队列默认从可用工作区顶部 48 vp 开始，安全边距为 8 vp，间距可选 `0 / 4 / 8` vp；横向对齐物理屏幕右边界
- 折叠成员静止时把 14 vp 的关闭段藏到屏幕外，Hover 时滑出；展开纸片对应的侧边标签保持完整可见
- 主胶囊固定在队列第 0 位，可纵向拖动整列；收起整列时移除成员行，只保留主胶囊；超出一屏的展开队列提供上一页 / 下一页按钮
...
折叠请求先进入非持久化的候选队列。只有父宿主就绪、当前布局和原生掩码完成、子窗口显示与交互调用成功后，才提交折叠状态并关闭原纸片。启动 / 掩码 / 显示失败或超时会保留原纸片，冷启动时已折叠的纸片会重新展开。失败会暂停自动重建，在设置中显式重试；不会把透明矩形窗口静默当成正常胶囊。

当前父宿主仍采用 3.3.7 的右下角一像素锚点方案，但会读回并校验实际位置，置顶作用于父主窗口。它是待真机验证的兼容方案，不是平台保证。普通悬浮胶囊仍由 PaperAbility 主窗口承载，必须单独验收。侧边 Hover 的位移动画暂时移除，避免动画与原生掩码不同步；形状不变时复用掩码，避免拖动整列时反复生成相同像素矩阵。
...
- Ability 进入后台、销毁或执行关键操作前强制 flush；保存失败会延迟重试；显式退出的任一次保存失败都会取消退出并显示重试入口
