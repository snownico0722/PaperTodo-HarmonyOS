...
| 自定义快捷键与队列数字键 | 明确不移植 | 保留固定全局快捷键；不提供逐项录入、小键盘区分、队列 `1–9` 快速打开和按鼠标位置新建 |
| 自定义字体与文字渲染档位 | 明确不移植 | 保留整体 / 分区字号与加粗；不加载外部字体文件，也不提供标准 / 柔和 / 锐利档位 |
| 图片标记显示档位 | 明确不移植 | 编辑与浏览沿用当前轻量 Markdown 表现，不增加标记始终显示 / 仅编辑显示 / 始终隐藏设置 |
| 外部打开后缀 | 明确不移植 | 固定交付临时 `.md`，不提供 `.txt` / `.html` 等自定义后缀 |
| 关联显示细分偏好 | 明确不移植 | 保留关联任意纸片和快启，不追加名称长度、路径精简等显示开关 |
| 高级设置、悬浮说明与精确阴影分层 | 明确不移植 | 设置保持三页直接呈现；视觉使用当前 ArkUI 阴影，不逐项复刻 Windows 层次 |
| 全屏避让与窗口切换隐藏 | 明确不移植 | 不识别外部全屏窗口，也不承诺控制 Alt+Tab、任务视图或 Dock 聚合行为 |
| Windows 托盘完整菜单、开机启动、CLI、PowerShell 脚本胶囊 | 平台专属，不移植 | HarmonyOS 使用系统状态栏扩展作为常驻入口，不复刻托盘逐纸片删除和无限滚动列表 |
...
- 多屏、多队列、左右换边、侧边胶囊队列排序、横向拖出以及对齐矩阵中标明的低优先级 Windows 对等设置不在移植范围内
