...
## 3.3.8 — 2026-09-05（可靠性修复候选）

- 胶囊折叠采用非持久化候选队列及实例 / 请求就绪确认；不再将 startAbility 接受请求视为可用窗口，原纸片保持展开直到交接成功。
- 首轮子窗口严格先布局 / 原生掩码，再显示 / 开启交互；移除固定 16ms 延迟与静默矩形透明回退。失败后恢复纸片并暂停自动重建，提供显式重试。
- 跨应用置顶改为父 UIAbility 主窗口；父宿主配置串行化，读回并校验右下角一像素锚点。旧实例、旧请求和超时后的完成回调不能确认新窗口就绪。
- 单队列加入工作区内分页，主胶囊 / 翻页控件 / 成员共用同一几何与掩码布局；保持原有稳定顺序，不新增多队列或拖拽排序。
- 移除侧边 Hover 位移动画，缓存相同原生掩码；运行中字体和关闭段设置变化会触发布局更新。
- 胶囊协调器不再把成功关闭窗口算作可达入口；隐藏已关闭纸片时不再新建 Ability 来执行隐藏。
- 显式退出在任一次保存失败时停止，保留 / 恢复状态栏并展示重试保存；设置始终提供恢复全部纸片，故障提示提供侧边胶囊重试。
- 新增生产逻辑的 Node 平台模拟回归和 Hypium 就绪 / 分页测试。Node 断言与 ArkTS 编译不代表真实窗口、系统置顶、点击穿透或覆盖升级已经通过。
- 更新版本与 README；详见 docs/V338_RELIABILITY_ACCEPTANCE.md。此版本仍需要 HarmonyOS PC / 2in1 真机验收，未宣称胶囊问题完全关闭。
